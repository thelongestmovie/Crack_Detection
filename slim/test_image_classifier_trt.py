...
import sys
sys.path.append('/home/yangyuhao/data/road/tftrt/example/tf_trt_models')
# print (sys.path)
import urllib
import tensorflow as tf
...
# with tf.Graph().as_default():
    ...
    image_batch, name_batch = tf.train.batch([processed_image, img_name], batch_size=batch_size, num_threads=32,
                                             capacity=2048,
                                             allow_smaller_final_batch=False)

    with open(os.path.join(FLAGS.test_dir, 'test.txt'), 'w') as f:
        tf_config = tf.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=1.0))
        ...
        threads = tf.train.start_queue_runners(tf_sess, coord)
        idx = 0
        ...
        try:
            while not coord.should_stop():
                time1 = time.time()
                print ('************')
                image_batch_v, name_batch_v = tf_sess.run([image_batch, name_batch])
                time_data = time_data + time.time()-time1
                time2 = time.time()
                # print (image_batch_v)
                predictions = tf_sess.run(tf_output, feed_dict={tf_input: image_batch_v})
                # print (predictions[0])
